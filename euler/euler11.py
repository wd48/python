# 11번 20×20 격자에서 연속된 네 수의 곱 중 최댓값

# 데이터 읽어오기 1 : 직접 변수에 선언
...
#         D1 = [numbers[i+k][i+k] for k in range(0,4)]
...
# 매개변수 x로 배열을 받으면, 배열 안의 모든 item을 다 곱해서 반환

# for i in range(17):
#     for j in range(17):
#         H = prod([int(k) for k in numbers[i][j:j+4]]) 
#         V = prod([int(numbers[i+k][j]) for k in range(0,4)])
#         D1 = prod([int(numbers[i+k][i+k]) for k in range(0,4)])
...
for i in range(17):
    for j in range(17):
        ...
        result.append(prod([int(numbers[i+k][i+k]) for k in range(0,4)]))
        result.append(prod([int(numbers[i+k][j+3-k]) for k in range(0,4)]))

print("4개 곱 중 최대값: ",max(result))